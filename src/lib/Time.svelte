...
<div class="textbox">
	<input style="background: {dark ? 'rgb(60,60,60)' : 'white'}; border-color: {dark ? 'var(--primary)' : 'var(--primary)'}; color: {dark ? 'var(--darktext)' : 'initial'};" type="time" id="{id}" name="{name}" placeholder="{placeholder}" required={required} bind:value={value} pattern={pattern} on:change on:keyup/>

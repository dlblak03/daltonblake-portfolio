...
<div class="textbox">
	<input style="background: {background};" type="password" id="{id}" name="{name}" placeholder="{placeholder}" disabled={disabled} required={required} bind:value={value} on:change on:keyup on:focus/>
	<label style="background: {background};" for="{id}"><slot /></label>
</div>

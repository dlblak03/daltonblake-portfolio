...
<div class="textbox">
	<input style="background: {dark ? 'rgb(60,60,60)' : 'white'}; border-color: {whitelabel ? primarycolor : 'var(--primary)'}; color: {dark ? 'var(--darktext)' : 'initial'};" type="date" id="{id}" name="{name}" placeholder="{placeholder}" required={required} bind:value={value} pattern={pattern} on:change on:keyup/>
